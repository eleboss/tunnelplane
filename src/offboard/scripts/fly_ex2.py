...
def callback_det(det):
    global target_x, target_y, target_z, target_rotation, target_yaw_bais, Received_det,last_target_rotation
    #相当于乘一个T
    camera_target = np.array([det.pose.position.z, -det.pose.position.x, -det.pose.position.y])
    ...
    target_x = camera_target_enu[0] - 0.13
    target_y = camera_target_enu[1]
    target_z = camera_target_enu[2] - 0.025
