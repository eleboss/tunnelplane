...
def callback_tag(tag):
    global tag_lost_counter, tag_yaw, tag_x, tag_y, tag_z, tag_seq, tag_time, KNOB
...
def callback_rc(rc):
    ...
    if rc.channels[9] > 0 and rc.channels[9] < 1000:
        KNOB = 0
    if rc.channels[9] >= 1000 and rc.channels[9] < 2000:
        KNOB = 1
    if rc.channels[9] >= 2000 and rc.channels[9] < 3000:
        KNOB = 2
...
while not rospy.is_shutdown():
    ...
    twist.twist.linear.x = output_x
    twist.twist.linear.y = output_y
    twist.twist.linear.z = output_z
    ...
    # print  "z_set:", pid_z.SetPoint, "z_out:", output_z, "pz_Feb:", feedback_z, "vz_Feb" ,local_linear_z ,'jump',jump_height 
